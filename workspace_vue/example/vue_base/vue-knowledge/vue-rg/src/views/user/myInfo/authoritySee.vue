    ...
    <div  class="min-height-100 Workbench" ref="Workbench">
        <Card :dis-hover="true" :bordered="false">
            ...
            <Card :dis-hover="true" :padding="0" class="eTh">
                ...
                <Row>
                    <Col span="24">
                        <div class="ApprovalState-text clearfix">
                            <span style="font-size: 14px;">如企业信息发生变更请及时更新</span><div class="right customBtn"><Button type="warning" @click="goback">重新认证</Button></div>{{entAuthInfos.entAuthReason}}
                        </div>
                    </Col>
                </Row>
            </Card>
        ...
        <Form type="flex"  :model="submitItem" ref="submitItem" :rules="ruleValidate" :label-width="200">
                    ...
                    <Row style="padding-top: 25px;">
                        <Col span="24">
                            <FormItem  label="企业类型：" prop="radio">
                                <span style="font-size: 12px;">{{submitItem.radio | filterType}}</span>
                            </FormItem>
                        </Col>

                        <Col span="24">
                            <FormItem  label="企业名称：" prop="eName">
                                <div>{{submitItem.eName}}</div>
                            </FormItem>
                        </Col>

                        <Col span="24">
                            <FormItem  label="统一社会信用代码：" prop="creditCode">
                                <span  placeholder="" class="w260">{{submitItem.creditCode}}</span>
                                <div class="creditCodeShow" v-if="creditCodeShow">
                                    <span style="padding-left: 15px;">有效期：</span>
                                    <span>{{submitItem.creditCodestartTime}}</span>
                                    <span>至</span>
                                    <span v-if="submitItem.creditCodeendTime !==null &&  submitItem.creditCodeendTime!==''">{{submitItem.creditCodeendTime}}</span>
                                    <span v-else>长期</span>
                                </div>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="24"  v-if="submitItem.radio === '8'">
                            <FormItem  label="药品生产许可证：" prop="DrugsLicence">
                                <span  placeholder="" class="w260">{{submitItem.DrugsLicence}}</span>
                                <span style="padding-left: 15px;">有效期：</span>
                                <span>{{submitItem.DrugsLicencestartTime}}</span>
                                <span>至</span>
                                <span>{{submitItem.DrugsLicenceendTime}}</span>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="24" v-if="submitItem.radio === '6' || submitItem.radio === '7'">
                            <FormItem  label="医疗机构执业许可证：" prop="MedicalLicence">
                                <span  placeholder="" class="w260">{{submitItem.MedicalLicence}}</span>
                                <span style="padding-left: 15px;">有效期：</span>
                                <span>{{submitItem.MedicalLicencestartTime}}</span>
                                <span>至</span>
                                <span>{{submitItem.MedicalLicenceendTime}}</span>
                            </FormItem>
                        </Col>

                        <Col span="24" v-if="submitItem.radio === '8'">
                            <FormItem  label="GMP证书："  prop="gspCertificate">
                                <span  placeholder="" class="w260">{{submitItem.gspCertificate}}</span>
                                <span style="padding-left: 15px;">有效期：</span>
                                <span>{{submitItem.gspstartTime}}</span>
                                <span>至</span>
                                <span>{{submitItem.gspendTime}}</span>
                        ...
                        <Col span="24" v-if="!eTypeE">
                            <FormItem label="所属区域:" prop="enterpriseAddressProvince">
                                {{submitItem.enterpriseAddressProvince}}
                                {{submitItem.enterpriseAddressCity}}
                                 {{submitItem.enterpriseAddressCountry}}
                            </FormItem>
                        </Col>
                        <Col span="24"  v-if="!eTypeE">
                            <FormItem  label="详细街道：" prop="dPosition">
                                <span>{{submitItem.dPosition}}</span>
                            </FormItem>
                        </Col>
                        <Col span="24"  v-if="eTypeE">
            ...
            <Card :dis-hover="true" :bordered="false" style="margin-top:-15px;">
                <Card :dis-hover="true" :padding="0">
                    <p slot="title" class="setTitile"><span class="fl">企业资质</span><span class="fr"></span></p>
                    <Row class="aptitude">
                        <Col span="24">
                            ...
                            <FormItem  label="GSP证书：" prop="imageUr3" v-if="submitItem.radio === '1' || submitItem.radio === '2' || submitItem.radio === '3' || submitItem.radio === '4' || submitItem.radio === '5'" class="back">
                                <div class="demo-upload-list" v-for="(item,index) in uploadList3">
                                    <template v-if="item.status === 'finished'">
                                        <img :src="item.url">
                                        <div class="demo-upload-list-cover">
                            ...
                            <FormItem  label="税票样本：" prop="imageUr10" v-if="submitItem.radio === '5' || submitItem.radio === '8'" class="back">
                            ...
                            <FormItem  label="医疗机构执业许可证：" prop="imageUr11" v-if="submitItem.radio === '6' || submitItem.radio === '7'" class="back">
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                ...
                ruleValidate: {
                    ...
                    imageUr3: [
                        {required: true, message: 'GSP证书图片必选', trigger: 'change'}
                    ],
                    imageUr4: [
        ...
        methods: {
            ...
            handleBeforeUpload1 (file) {// 上传前钩子函数
                let that = this;
                ...
                this.$https.post('/api-webbff/v1/member/getEntInfoByEntId2RGT', {
                    ...
                    .then(res => {
                        if (res.data.success === 1) {
                            ...
                            self.submitItem.enterpriseAddressProvince = entinfo.eprovinceName;
                            self.submitItem.enterpriseAddressCity = entinfo.ecityName;
                            self.submitItem.enterpriseAddressCountry = entinfo.eregionName;
        ...
        mounted () {
            this.$store.commit('setAuthSeeShow', true)
...
// WEBPACK FOOTER //
// src/views/user/myInfo/authoritySee.vue