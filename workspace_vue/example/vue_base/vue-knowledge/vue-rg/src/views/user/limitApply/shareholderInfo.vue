    ...
    <div>
        ...
        <Collapse class="collapseSetStyle" v-model="collapseValue" :accordion="true">
            <Panel name="1">
                填写股东信息
                <div slot="content">
                    <Button type="primary" :disabled="disableApplyBtn" @click="addShareholderOne(1, {enterpriseCreditApplyId: creditApplyIds, shareholderType: 2}, 1)">添加股东</Button>
...
<script>
    ...
    let shareholderTh = [
        {key: 'id', title: '序号', fixed: 'left', width: 100, type: 'index', align: 'center', className: 'col-no-padding'},
        {
            ...
            render: (h, params) => {
                let btn1= h('span', {
                    ...
                    on: {
                        click: (ev) => {
                            vm.addShareholderOne(1, params.row, 2)
                        }
                    }
                }, '修改');
                let btn2 = h('span', {
                    ...
                    on: {
                        click: (ev) => {
                            vm.shareholderDel(params.row.shareholderId, 1, params.row.isHaveSubShareholders)
                        }
                    }
                }, '删除');
                let btn3 = h('div', {
                    ...
                    on: {
                        click: (ev) => {
                            vm.addShareholderOne(2, params.row, 1)
                        }
                    }
                }, '新增股东的股东');
                if (params.row.stats === 2) {
                    return h('div', [
                        btn2
                    ]);
                } else {
        ...
        },
        {
            ...
            className: 'col-no-padding',
            render: (h, params) => {
                if (((vm.getEntTypeLsds === '1' && vm.getClassifications === 3)||(vm.getEntTypeLsds==='2'))&&(params.row.shareholderType === 2)) {
                    return h('a',{
                        domProps: {
        ...
        {key: 'id', title: '序号',  fixed: 'left', width: 80, type: 'index', align: 'center', className: 'col-no-padding'},
        {
            ...
            render: (h, params) => {
                let btn1= h('span', {
                    ...
                    on: {
                        click: (ev) => {
                            vm.addShareholderOne(2, params.row, 2)
                        }
                    }
                }, '修改');
                let btn2 = h('span', {
                    ...
                    on: {
                        click: (ev) => {
                            vm.shareholderDel(params.row.shareholderId, 2, params.row.isHaveSubShareholders)
                        }
                    }
                }, '删除');
                if (params.row.stats === 2) {
                    ...
                    .then(res => {
                        if (res.data.success === 1) {
                            if (rank === 1) {
                                ...
                                let isHaveSubShareholders = that.shareholderOneLevel.find((item,index,arr) => item.shareholderType === 1&&item.isHaveSubShareholders==='0');
                                if (isHaveSubShareholders !== undefined) {
                                    this.$emit('isHaveSubShareholders', false, isHaveSubShareholders.shareholderName);
                                } else {
                                    this.$emit('isHaveSubShareholders', true, '');
                                }
                            } else {
...
// WEBPACK FOOTER //
// src/views/user/limitApply/shareholderInfo.vue